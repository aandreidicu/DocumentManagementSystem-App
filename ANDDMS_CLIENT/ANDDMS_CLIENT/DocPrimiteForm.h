...
namespace ANDDMSCLIENT {
	...
	/// </summary>
	public ref class DocPrimiteForm : public System::Windows::Forms::Form
	{
	public:
		Pachet^ clientul;
		DocPrimiteForm(Pachet^ c)
		{
			//listaDocumente = GetDoc();
			clientul = c;
			InitializeComponent();
			//
		...
		{
			auto list = gcnew List<Document^>();
			
			list->Add(gcnew Document("s", "titlu meu", "exp", "data", "ora", "continut"));
			
			return list;
		}

	public: void DocPrimiteForm_Load(Object sender, EventArgs e)
		{
		auto documente = this->listaDocumente;
		this->dataGridView1->Rows->Add("titlu", "exp", "data", "ora");
		}*/
	...
	private: System::Windows::Forms::DataGridViewTextBoxColumn^ EXPEDITOR;
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->EXPEDITOR = (gcnew System::Windows::Forms::DataGridViewTextBoxColumn());
			...
			this->panel1->Size = System::Drawing::Size(1084, 538);
			...
			this->dataGridView1->Columns->AddRange(gcnew cli::array< System::Windows::Forms::DataGridViewColumn^  >(6) {
				this->TITLU,
					this->ID, this->EXPEDITOR, this->DATA, this->ORA, this->DESCARCA
			});
			...
			this->dataGridView1->CellClick += gcnew System::Windows::Forms::DataGridViewCellEventHandler(this, &DocPrimiteForm::dataGridView1_CellClick);
			this->dataGridView1->CellContentClick += gcnew System::Windows::Forms::DataGridViewCellEventHandler(this, &DocPrimiteForm::dataGridView1_CellContentClick);
			this->dataGridView1->CellDoubleClick += gcnew System::Windows::Forms::DataGridViewCellEventHandler(this, &DocPrimiteForm::dataGridView1_CellDoubleClick);
			// 
			// TITLU
			// 
			...
			// EXPEDITOR
			// 
			this->EXPEDITOR->HeaderText = L"EXPEDITOR";
			this->EXPEDITOR->Name = L"EXPEDITOR";
			this->EXPEDITOR->ReadOnly = true;
			// 
			// DATA
			// 
			...
			this->Name = L"DocPrimiteForm";
			this->ShowInTaskbar = false;
			this->Load += gcnew System::EventHandler(this, &DocPrimiteForm::DocPrimiteForm_Load_1);
		...
		/*public:System::Void actualizare_DocumentePrimite(char* num)
		{
			String^ cont;
			if (strcmp(num, clientul->c->getUser()) == 0)
			{
				for (list<CDocument>::iterator p = clientul->c->get_docPrimiteClient()->get_docPrimite()->begin(); p != clientul->c->get_docPrimiteClient()->get_docPrimite()->end(); p++)
				{
					String^ g = gcnew String((char*)p->get_continut());
					cont = gcnew String(clientul->c->getUser()) + ":" + g;

					this->dataGridView1->Rows->Add(cont);
				}
			}
		}*/
		/*void DocPrimiteForm_Load(System::Object^ sender, System::EventArgs^ e)
		{
			
			for (std::list<CDocument>::iterator i = clientul->c->docPrim.get_docPrimite()->begin(); i != clientul->c->docPrim.get_docPrimite()->end(); i++)
			{
				String^ ids;
				...
				String^ continuts;
				continuts = marshal_as<String^>(i->continut);

				this->dataGridView1->Rows->Add(titlus, expeditors, datas, oras);
			}
		}*/
...
#pragma endregion
	...
	private: System::Void DocPrimiteForm_Load_1(System::Object^ sender, System::EventArgs^ e) {
		...
		//	
		//}
		for (std::list<CDocument>::iterator i = clientul->c->get_docPrim()->get_docPrimite()->begin(); i != clientul->c->get_docPrim()->get_docPrimite()->end(); i++)
		{
			String^ ids;
			ids = marshal_as<String^>(i->id);
			String^ titlus;
			...
			this->dataGridView1->Rows->Add(titlus, ids, expeditors, datas, oras);
		}
...
private: System::Void dataGridView1_CellDoubleClick(System::Object^ sender, System::Windows::Forms::DataGridViewCellEventArgs^ e) {
	...
	if (clientul->flagafis == true && clientul->c->get_afisProv() != "") {
		...
		if (titlu == "Permisie" || titlu == "Raport_de_parasire_a_tarii") {
			if (MessageBox::Show(cont, "Acceptati documentul?", MessageBoxButtons::YesNo, MessageBoxIcon::Question) == ::DialogResult::Yes)
			{
				clientul->c->F_ACCEPTA_DOCUMENT(id, clientul->c->getUsername());
			}
			else
			{
				clientul->c->F_RESPINGE_DOCUMENT(id, clientul->c->getUsername());
			}
		}
		else
		{
